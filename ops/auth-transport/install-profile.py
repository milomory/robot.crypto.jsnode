#!/usr/bin/python3 -I
"""Install the fixed Crypto Auth transport profile using metadata only."""
import datetime
import json
import os
...
def main():
    ...
    backup = Path('/var/lib/agent-secrets-broker/backups') / ('crypto-auth-transport-' + stamp)
    backup.mkdir(mode=0o700)
    shutil.copy2(registry, backup / 'registry.before.json')
    ref = 'secret://inbox/web-20260904t230513z-hlpfublwbctp'
    destination = 'hyperion.crypto-auth-transport'
    profile = 'hyperion-crypto-auth-transport-deploy'
    ...
    for name, installed in [('crypto-auth-transport-bridge.py', 'crypto-auth-transport-bridge.py'), ('crypto-auth-transport-remote.py', 'crypto-auth-transport-remote.py')]:
        target = Path('/usr/local/libexec') / installed
        if target.exists() or target.is_symlink():
            raise RuntimeError('helper path occupied')
        shutil.copyfile(source / name, target)
        target.chmod(0o755)
    sudoers = Path('/etc/sudoers.d/93-crypto-auth-transport-deploy')
    if sudoers.exists():
        raise RuntimeError('sudoers path occupied')
    sudoers.write_text('anton ALL=(root) NOPASSWD: /usr/local/libexec/crypto-auth-transport-bridge.py --server\n')
    sudoers.chmod(0o440)
    ...
    data['profiles'][profile] = {
        ...
        'executable': '/usr/local/libexec/crypto-auth-transport-bridge.py',
        'injection': 'stdin', 'timeout_seconds': 120,
    }
    stage = registry.with_name('athena-registry.crypto-robot-stage.json')
    if stage.exists():
