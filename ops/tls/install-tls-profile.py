#!/usr/bin/python3 -I
"""Install the fixed Crypto Robot TLS profile using metadata only."""
import datetime
import json
import os
...
def main():
    ...
    for name, installed in [('crypto-tls-bridge.py', 'crypto-tls-bridge.py'), ('crypto-tls-remote.py', 'crypto-tls-remote.py')]:
        target = Path('/usr/local/libexec') / installed
        if target.exists() or target.is_symlink():
            raise RuntimeError('helper path occupied')
        shutil.copyfile(source / name, target)
        target.chmod(0o755)
    sudoers = Path('/etc/sudoers.d/93-crypto-robot-tls-deploy')
    if sudoers.exists():
        raise RuntimeError('sudoers path occupied')
    sudoers.write_text('anton ALL=(root) NOPASSWD: /usr/local/libexec/crypto-tls-bridge.py --server\n')
    sudoers.chmod(0o440)
    ...
    data['profiles'][profile] = {
        ...
        'executable': '/usr/local/libexec/crypto-tls-bridge.py',
